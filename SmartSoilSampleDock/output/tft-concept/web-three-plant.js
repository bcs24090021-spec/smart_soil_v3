...
export class DigitalTwin {
  ...
  buildSoil(scene) {
    const wood = new THREE.MeshStandardMaterial({ color: 0x9a7a52, roughness: 0.9 });
    const frameMat = new THREE.MeshStandardMaterial({ color: 0x8a6b47, roughness: 0.9 });
    const corners = [
      [-1, -1], [1, -1], [-1, 1], [1, 1],
    ];
    for (const [cx, cz] of corners) {
      const col = new THREE.Mesh(new THREE.BoxGeometry(0.09, SOIL.h + 0.08, 0.09), wood);
      col.position.set((SOIL.w / 2) * cx, SOIL.h / 2, (SOIL.d / 2) * cz);
      scene.add(col);
    }
    // translucent soil block with a natural texture (transparent cross-section)
